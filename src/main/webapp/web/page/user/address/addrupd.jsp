...
<body>
    ...
    <form class="layui-form" lay-filter="updaddr" method="post">
...
function init(){
	...
	$.ajax({
        url:con.app+'/addr/search.do',
        data : {id:id},
        dataType : 'json',
        type : 'post',
        success : function(data) {
        	form.val("updaddr", 
        		{userCode: data.userCode,name: data.name,tel: data.tel,province: data.province,city: data.city,district: data.district,address: data.address})
        }
    })
}
formSubmit('/addr/upd.do', 'submit(upd)', 'text', function(data) {
	if (data == 1) {
        layer.msg('修改成功');
        closeThis(2000);
    } else {
